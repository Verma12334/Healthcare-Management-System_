...
require_once 'connect.php';
    ...
    // $servername = "localhost";
      ...
      try {
        ...
        // Step 2: Insert into the `staff` table using the max `eid`
        $sql = "INSERT INTO $myDB.staff (sid, type) VALUES (:sid, :type)";
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':sid', $maxEid, PDO::PARAM_INT);       // Bind max `eid` as `sid`
        $stmt->bindParam(':type', $type, PDO::PARAM_STR);         // Bind the correct variable `$type`
